...
D1 = 21 + 500/s + 0.15*s;
D2 = 11/s*(s+50)*(1/80*s+1);
D3 = (1/s)*450*((s/40+1)/(s/2000+1))^2;

figure
step(feedback(D1*motor,1));
hold on
step(feedback(D2*motor,1));
step(feedback(D3*motor,1));
hold off
ylabel('Position, \theta (radians)');
title('Response to a Step Reference');
legend('step response',  'root locus',  'bode plot','location','southeast');

figure
step(feedback(motor,D1));
hold on
step(feedback(motor,D2));
step(feedback(motor,D3));
hold off
ylabel('Position, \theta (radians)');
title('Response to a Step Disturbance');
legend('step response',  'root locus',  'bode plot','location','southeast');
